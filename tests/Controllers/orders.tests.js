describe('ControllersOrder', function() {
   ...
   // scope.signin();
    ...
    expect(scope.orders[5].totalPrice).toEqual(3320);
     ...
     it('should be able to get a Order By ID', function() {
        // Create new Order model instance
         ...
         $httpBackend.expectGET('http://opos.tech-dojo.org/orders/'+ stateParams.orderId).respond(200, i); //Fake
    ...
    $httpBackend.expectGET('http://opos.tech-dojo.org/fooditems').respond(200, j); //Fake
   ...
   // console.log ('Hello Orders 2 ' +scope.orders[5].totalPrice);
    ...
    $httpBackend.expectGET('http://opos.tech-dojo.org/setmenus').respond(200, l); //Fake
